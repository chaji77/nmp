...
public class Tax extends BaroBill {

    public Tax() throws MalformedURLException {
        super();
    }
    ...
    public int RegistAndIssueTaxInvoice(InvoiceVO vo) throws RemoteException {
      int intResult = 0;
      try {
        super.setInstance();
        ...
        taxInvoice.getInvoicerParty().setCorpNum(super.cn);        //필수입력 - 연계사업자 사업자번호 ('-' 제외, 10자리)
        taxInvoice.getInvoicerParty().setTaxRegID("");
        taxInvoice.getInvoicerParty().setCorpName(super.cnm);        //필수입력
        taxInvoice.getInvoicerParty().setCEOName(super.ceo);        //필수입력
        taxInvoice.getInvoicerParty().setAddr(super.p_addr1);
        taxInvoice.getInvoicerParty().setBizType(super.p_bizType);
        taxInvoice.getInvoicerParty().setBizClass(super.p_bizClass);
        taxInvoice.getInvoicerParty().setContactID(super.cid);        //필수입력 - 담당자 바로빌 아이디
        taxInvoice.getInvoicerParty().setContactName(super.p_memberName);    //필수입력
        taxInvoice.getInvoicerParty().setTEL("");
        taxInvoice.getInvoicerParty().setHP("");
        taxInvoice.getInvoicerParty().setEmail(super.p_email);            //필수입력
        ...
        taxInvoice.getInvoiceeParty().setContactName(vo.strToManager); //필수입력
    ...
    public void GetTaxInvoicePopUpURL(String strMgtKey) {
        try {
            String certKey = super.ck;           //인증키
            String corpNum = super.cn;           //연계사업자 사업자번호 ('-' 제외, 10자리)
            // String mgtKey = "";                //연동사부여 문서키
            ...
            String result = barobillApiService.taxInvoice.getTaxInvoicePopUpURL(certKey, corpNum, strMgtKey, id, pwd);
