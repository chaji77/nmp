...
function checkFormField() {
  ...
  if (frm.strToTaxRegId.value.length > 0) {
      var taxId = frm.strToTaxRegId.value;
      while(taxId.length < 4) {
          taxId = "0" + taxId;
      }
      frm.strToTaxRegId.value = taxId;
  }
  ...
  <table class='list detail'>
    ...
    <tbody class='obj'>
      <tr>
        ...
        <td><input type='number' name='bizno' value='<%=vo.strToBizNo%>' maxlength=8 placeholder='숫자만 입력하십시오.' onKeyDown="onlyNumber();"></td>
        <th class='blue'>종사업장번호</th>
        <td><input type='text' name='strToTaxRegId' value='<%=StrUtil.nvl(vo.strToTaxRegId, "")%>' maxlength=4 placeholder='숫자만 입력하십시오.' onKeyDown="onlyNumber();"></td>
        </tr>
      <tr>
        <th class='red'>상　호</th>
      ...
      <tr>
        <th class='red'>주　소</th>
        <td colspan='3' style='white-space:normal;'><input type="text" name="invoicer_addr" value="<%= vo.INVOICER_ADDR != null ? vo.INVOICER_ADDR : "" %>" maxlength="150"></td>
