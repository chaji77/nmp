...
String strErrorReturn = "<script>alert('사업자번호를 확인할 수 없습니다. 다시 시작하십시오.');location.href='Regist.jsp';</script>";
if (!IntegerCryptoUtil.isEncrypted(strBizNo)) {
  out.println(strErrorReturn);
  return;
}
if(referer == null || referer.length()==0 || !referer.contains("/firstbill/Regist")) {
  out.println(strErrorReturn);
  return;
}
%>
...
function check() {
  ...
  if (!validate("input[name='login_tel']", "length", [9,14], "전화번호를 입력하십시오.")) return false;
...
function callback() {
  toast("가입되었습니다. 로그인하십시오.", 2000, function() {location.href = "index.jsp";});
}
...
function initDuplicateCheck() {
  $("input[name='login_id']").focus();
  $("input[name='login_id']").prop("readonly", false);
  $("#login-id-duplicate-check-btn").html("<a onclick='checkDuplicated();' class='btn'>중복확인</a>");
  document.frmEnt.checkDuplicated.value = 'N';
}
...
<div class='page-title-block'>
  <span class='title'>처음빌 회원가입</span>
  <span class='more'>
  </span>
</div>

<ul class='step'>
  <li><strong>STEP 1</strong><br/>회원가입여부<br/>확인</li><li class='after'></li>
  <li><strong>STEP 3</strong><br/>회원정보 입력</li><li class='after'></li>
  <li><strong>STEP 4</strong><br/>회원가입 완료</li>
</ul>
...
<ul class='form'>
  ...
  <li>
    ...
    <input type='tel' name='login_tel' value='' maxlength='13' placeholder='전화번호' required style='margin-bottom:5px;'>
  </li>
  <li>
    <label>메일주소 <i class="fa-solid fa-asterisk"></i></label>
