...
function addRow() {
  ...
  // 세액은 자동계산되지만, 사용자가 직접 수정도 가능 → 변경 시 합계만 재계산
...
// 현재 선택된 과세구분에 따른 세율 반환
// 일반(Y) : 0.1, 면세(F), 영세(Z) : 0
function getTaxRate() {
  var t = $("input:radio[name='taxtype']:checked").val();
  if (t === "Y") {
    return 0.1;
  }
  return 0;
...
<form name='frmEnt' method='post' action='InvoiceProc.jsp'>
  ...
  <table class='list detail'>
    ...
    <tbody>
      <tr>
        ...
        <td>
          <!-- 과세구분 바뀌면 전체 세액 재계산 -->
          <input type='radio' name='taxtype' id='taxtype_general' value='Y' onclick='refreshTaxAll()' checked> 일반
          <input type='radio' name='taxtype' id='taxtype_free'    value='F' onclick='refreshTaxAll()'> 면세
          <input type='radio' name='taxtype' id='taxtype_zero'    value='Z' onclick='refreshTaxAll()'> 영세
        </td>
        <th>영수/청구 구분</th>
        <td>
  ...
  <table class='list detail'>
    ...
    <tbody>
      ...
      <tr>
        <th>비고</th>
        <td colspan="3"><input type='text' name='remark' value='' maxlength='100'></td>
      </tr>
    </tbody>
  </table>
