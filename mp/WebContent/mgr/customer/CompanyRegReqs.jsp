...
function goPage(p) {
  ...
  document.frmSearch.action = "CompanyRegReqs.jsp";
...
function regNew() {
  $("#element_to_pop_up").bPopup({loadUrl:'<%=request.getContextPath()%>/mgr/customer/CompanyRegReqReg.jsp'});
}
function modifyReq(obj) {
  const tr = $(obj).closest('tr');
  $("#element_to_pop_up").bPopup({
    loadUrl: 'CompanyRegReqReg.jsp'
        + '?seq='         + encodeURIComponent(tr.data('seq'))
        + '&buyCpyId='    + encodeURIComponent(tr.data('buyCpyId'))
        + '&buyCpyName='  + encodeURIComponent(tr.data('buyCpyName'))
        + '&sellCpyName=' + encodeURIComponent(tr.data('sellCpyName'))
        + '&sellPhone='   + encodeURIComponent(tr.data('sellPhone'))
        + '&sellPrsName=' + encodeURIComponent(tr.data('sellPrsName'))
        + '&sellFax='     + encodeURIComponent(tr.data('sellFax'))
        + '&sellEmail='   + encodeURIComponent(tr.data('sellEmail'))
        + '&bizNo='       + encodeURIComponent(tr.data('bizNo'))
        + '&tradeDate='   + encodeURIComponent(tr.data('tradeDate'))
        + '&feePay='      + encodeURIComponent(tr.data('feePay'))
        + '&memo='        + encodeURIComponent(tr.data('memo'))
        + '&reqStatus='   + encodeURIComponent(tr.data('reqStatus'))
  });
}
function dropReq(obj) {
  const seq = $(obj).closest('tr').data('seq');
  showCustomConfirm("정말 삭제하시겠습니까?", function () {
    $.post("CompanyRegReqModProc.jsp", {seq: seq, mode: 'drop'}, function(data) {
...
%>
        ...
        <li>
          <label>상호명</label>
          <input type='text' name='searchNm' value='<%=pvo.SEARCH_NM%>' placeholder='판매기업명/구매기업명'>
        </li>
      </ul>
    </td>
    <td class='fill'></td>
    <td class='btn' style='text-align:right;'>
      ...
      <a href='CompanyRegReqs.jsp'><i class="fa-solid fa-rotate-right" style='font-size:1.7em;margin-right:10px;'></i></a>
    </td>
  </tr>
</tbody>
</table>
...
%>
    <tr
        data-seq='<%=v.SEQ%>'
        data-buy-cpy-id='<%=v.BUY_CPY_ID%>'
        data-buy-cpy-name='<%=StrUtil.input(v.BUY_CPY_NAME)%>'
...
} else out.println("<tr><td colspan='12' class='noentry'>조회된 등록요청이 없습니다.</td></tr>");
%>
  </tbody>
</table>
<div id="paging">
