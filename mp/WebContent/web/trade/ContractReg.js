...
function calcItem(obj) {
  ...
  if (booScrap) { 
    ...
    taxrate    = 1;   
    if ($(obj).parent().parent().find(".item_tax").text()==0) amount = sum; 
    else { 
...
// 품목정보공란점검 & 거래불가품목점검 & 0원/마이너스 금액 점검
  var blockKeywords = ["계약", "선급", "공사", "인건비", "인력", "임대", "수수료", "식대", "수선비", "유지보수", "시스템"];
  var blockedItems  = [];
  var hasZeroMinus  = false;

  $("#bill-item-list tr td input[name='item_nm']").each(function(idx, item) {
    $(item).css("border", "1px solid #ddd");
    var word = $.trim($(item).val());
    ...
    for (var i = 0; i < blockKeywords.length; i++) {
      if (word.indexOf(blockKeywords[i]) > -1) {
        blockedItems.push({ nm: word, kw: blockKeywords[i] });
        break;
      }
    }
    ...
    var sumVal = $tr.find("input[name='item_sum']").val();
    var numSum = parseInt((sumVal || '0').replace(/,/g, ''), 10);
  ...
  if (!is) return false;
   ...
   // 거래불가 품목 alert 알림 — 진행 불가
  if (blockedItems.length > 0) {
    var kwList = [];
    $.each(blockedItems, function(i, v) {
      if (kwList.indexOf(v.kw) < 0) kwList.push(v.kw);
    });
    var kwStr    = '"' + kwList.join('", "') + '"';
    var blockMsg = "[거래 불가 품목 안내]\n\n"
      + "보증기관 및 금융기관에서는 원칙적으로 거래의 물품 또는 용역의 내역을 자세히 제시하지 않거나 "
      + "경상적 영업활동과 무관한 " + kwStr + " (해당 품목) 거래를 금지하고 있습니다.\n\n"
      + "품목을 수정 후 다시 시도하십시오.";
    showErrorMsg(blockMsg);
    return false;
  }

  // 0원/마이너스 금액 알림
  if (hasZeroMinus) {
    showErrorMsg("[매매계약서 작성 내용 확인 안내]\n\n"
      + "품목 금액이 0원 또는 마이너스 금액이 있습니다.\n"
      + "0원 품목 및 마이너스 품목은 삭제 후 다시 작성 부탁드립니다.");
    return false;
  }
  ...
  if (check()) {
    ...
    $.ajax({
      ...
      success: function(data) {
		...
		if ($("input[name='temp_save']").val() == "1") {
		  showAlert("임시저장 되었습니다.", function () {
		    location.reload();  
		  });
		  return;
		}
        if (isNaN(data)) {
          ...
          document.frmEnt.action = strContextPath + "/web/trade/ContractsSent.jsp";
