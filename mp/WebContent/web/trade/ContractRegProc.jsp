...
if (headerVo.TAXBIZTYPE.equals("G") || headerVo.CU_USE_YN.equals("Y")) {
  ...
  if (headerVo.CU_USE_YN.equals("Y")) {
    headerVo.TAXAMT = "0";
    headerVo.SUPPLYAMT = headerVo.TOTALCONTRACTAMT;
  }
}
...
if (headerVo.CTID.equals("0")) { // INSERT
  ...
  if (rvo != null && !StrUtil.nvl(rvo.CTID).equals("0")) {
   try {
     MpFeeCalcurator feeCalc = new MpFeeCalcurator();
     feeCalc.execute(Integer.parseInt(rvo.CTID));
    
     rvo = feeCalc.getCtHeaderFormat(rvo);
     bean.CT_HEADER_MOD_PROC(rvo, strItemXml); 
     
   } catch (Exception e) {
       logger.error("임시저장 수수료 재계산 에러: " + e.toString());
   }
  }
}
if (rvo==null) {
