...
String strSeq = StrUtil.nvl(request.getParameter("seq"), "0");
if (!StrUtil.isOnlyNumeric(strSeq) || strSeq.equals("0")) {
  out.println(0);
  return;
}

if ("drop".equals(request.getParameter("mode"))) {
  out.println(new RegReqBean().COMPANY_REG_REQ_DROP_PROC(Integer.parseInt(strSeq)));
  return;
}

RegReqVO vo = new RegReqVO();
vo.SEQ                  = Integer.parseInt(strSeq);
...
vo.REQ_STATUS           = (StrUtil.isOnlyNumeric(strReqStatus)) ? Integer.parseInt(strReqStatus) : 1;

out.println(new RegReqBean().COMPANY_REG_REQ_MOD_PROC(vo));
%>