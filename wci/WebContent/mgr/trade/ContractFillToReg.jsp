...
/************** PAYMENT **************/
console.log('<%=vo.BNK_CD %>____<%=vo.PAY_ID %>');
...
%>
  ...
  $("#items tfoot").show();
  // $("div.btns").html("<a onclick='save(loadCert);'><i class='fa fa-paper-plane'></i> &nbsp;승인</a>");
<%
}
%>

/************** MANAGER **************/
$("input[name='buyer_ip']").val("<%=vo.BUYER_IP%>");
$("input[name='seller_ip']").val("<%=vo.SELLER_IP%>");
$("input[name='reg_id']").val("<%=vo.REGUSER%>");
$("input[name='buyer_sgn_id']").val("<%=vo.SGN_ID %>");
$("input[name='seller_sgn_id]").val("<%=vo.SELLER_APP_SGN_ID %>");
