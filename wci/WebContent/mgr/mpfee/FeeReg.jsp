...
<style>
table.fee_conditions {table-layout: fixed;margin-left:144px;width:calc(100% - 144px);}
table.fee_conditions th, table.fee_conditions td {padding-left:0;background-color:white;}
table.fee_conditions th {border-bottom: 1px dotted #ddd;}
table.fee_conditions td {padding-top:2px;padding-bottom:2px;}
@media only screen and (max-width:767px) {
  table.fee_conditions {margin-left:0px;width:100%;min-width:500px;}
  table.fee_conditions input {max-width: 30px;}
}
</style>
<script>
...
function toogleMethod() {
  ...
  if ("A10,C10,B10,B20".indexOf(method)>-1) {
    ...
    $("table.fee_conditions tbody tr").show();
    ...
    if (method=="B10" || method=="B20") {
      $(".commission_rate").show();
      $("table.fee_conditions tbody tr").hide();
      $("table.fee_conditions tbody tr").eq(1).show();
    }
  }
  else if (method=="F10") {
    ...
    $("input[name='max_yn']").prop("checked", false);
    $(".term, .receive_money, .offline_yn").show();
  }
  if ($(".offline_yn").is(":visible")==false) $("input[name='offline_yn']").prop("checked", false);
}
...
function fillData() {
  ...
  $("input[name='symd']").val("<%=FormatUtil.addSeparatorDate(vo.START_DT, "-") %>");
  $("input[name='eymd']").val("<%=FormatUtil.addSeparatorDate(vo.END_DT, "-") %>");
...
<ul class='form'>
  <li>
    <label>구매사</label>
    <input type='search' name='buyer_nm' value='<%=strCpyNm %>' style='width:calc(100% - 186px);' readOnly placeholder='선택하지 않으면 모든 구매사로 적용' onclick='searchCompany(0);'>
    <a onclick='searchCompany(0);' class='btn search-company'>검색</a>
  </li>
  <li>
    <label>판매사</label>
    <input type='search' name='seller_nm' style='width:calc(100% - 186px);' readOnly placeholder='선택하지 않으면 모든 판매사로 적용' onclick='searchCompany(1);'>
  ...
  <li class='term'>
    <label>적용기간</label>
    <input type='date' name='symd' style='width:85px;'> ~ <input type='date' name='eymd' style='width:85px;'>
  </li>
</ul>

<table class='fee_conditions list'>
  <thead>
    <tr>
      <th class='std_days left'>기준일수</th>
      <th class='mty_days left'>만기계산일수</th>
      <th class='commission_rate left'>기본수수료율(%)</th>
      <th class='discount_rate left'>할인율</th>
      <th class='commission_rate_etc left'>비례수수료율1, 2(%)</th>
    </tr>
  </thead>
  <tbody>
<%
...
%>
    <tr>
      <td class='std_days left'><input type='number' name='std_days_<%=i %>' style='width:85px;' placeholder='기준일수' value='<%=((i*30)+1)%>'></td>
      <td class='mty_days left'>
        <input type='number' name='mty_stdays_<%=i %>' style='width:85px;' placeholder='시작일수' value='<%=((i*30)+1)%>'> ~ 
        <input type='number' name='mty_enddays_<%=i %>' style='width:85px;' placeholder='종료일수' value='<%=((i+1)*30)%>'>
      </td>
      <td class='commission_rate left'><input type='number' name='cpy_commission_rate_<%=i %>' style='width:85px;' placeholder='기본율'></td>
      <td class='discount_rate left'><input type='number' name='discount_rate_<%=i %>' style='width:85px;' placeholder='할인율' value='0'></td>
      <td class='commission_rate_etc left'>
        <input type='number' name='cpy_commission_rate1_<%=i %>' style='width:85px;' placeholder='비례율1'>
        <input type='number' name='cpy_commission_rate2'_<%=i %> style='width:85px;' placeholder='비례율2'>
      </td>
    </tr>
<%
}
...
<ul class='form' style='margin-top:10px;'>
