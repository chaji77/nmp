...
try {
  cvo.CPY_ID = 0;
  cvo.CPY_BUSINESS_NO = StrUtil.extractDigits(request.getParameter("bizno"), 10);
  ...
  if (!isError) {
    ...
    int intDuplicated = bean.COMPANY_BIZNO_CHECK_PROC(cvo.CPY_BUSINESS_NO);
    if (intDuplicated>0) {
      isError = true;
      strErrorMsg = "이미 가입된 사업자입니다.";
    }
    if (intDuplicated==0) {
      intDuplicated = bean.COMPANY_LOGIN_ID_CHECK_PROC(pvo.PRS_LOGIN);
      if (intDuplicated>0) {
        isError = true;
    ...
    if (intDuplicated==0) {
      intCpyId = bean.COMPANY_ADD_PROC(cvo, pvo);
      if (intCpyId==0) {
        isError = true;
        strErrorMsg = "가입할 수 없습니다. 고객센터로 문의바랍니다.";
      } else {
        session.setAttribute("REGISTED_CPY_ID", intCpyId);
        session.setAttribute("REGISTED_CPY_NM", cvo.CPY_NAME);
        session.setAttribute("REGISTED_FILE",   cvo.BIZ_DOC_FILE_URL);
      }
    }
  }
  ...
  isError = true;
  strErrorMsg = "등록할 수 없습니다. 다시 시도하십시오. 문제가 지속되면 고객센터로 문의바랍니다.";
}
