...
import kr.co.soap.kodit.loan.Kodit_K311;

public class EmtNetK311 extends Kodit_K311 {
	...
	protected String validateByK3XX() throws Exception {
		...
		System.out.println("this.ctheaderVO.STATUS : " + this.ctheaderVO.STATUS);
	...
	protected void processByK3XX() throws Exception {
	    ...
	    try {
	    	...
	    	if ("060".equals(headerVO.STATUS)) {
	    		...
	    		if (!this.ctheaderVO.BNKCD.equalsIgnoreCase("TB")) TalkCtrl.sendBySystem("M016", 0, this.ctheaderVO.CTID, 0);	// 판매사 추심완료 후 구매사에게 카카오톡 전송 
	    	}
	    }
	    catch (Exception e)
	    {
